...
int main(){
	...
	for(int i=0;i<filas-2;i++){
		for(int j=0;j<col-2;j++){			
			cout<<"pos"<<i<<j;
			if(car[i][j]<255){
				cmod[imod][jmod]=abs((car[i][j]+car[i+1][j]+car[i+2][j])-(car[i][j+2]+car[i+1][j+2]+car[i+2][j+2]));
				cmod2[imod][jmod]=abs((car[i][j]+car[i][j+1]+car[i][j+2])-(car[i+2][j]+car[i+2][j+1]+car[i][j+2]));
				cmod[imod][jmod]+=cmod2[imod][jmod];
				cmod[imod][jmod]/=2;
				cmod2[imod][jmod]=cmod[imod][jmod];
				if(cmod[imod][jmod]>40){
					cmod[imod][jmod]=(255+cmod2[imod][jmod]+car[i][j])/3;
				}
			}
